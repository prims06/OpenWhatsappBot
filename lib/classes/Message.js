...
class Message {
  ...
  async downloadQuotedMedia() {
    try {
        const context = this.data.message?.extendedTextMessage?.contextInfo;
        const quoted = context?.quotedMessage;

        if (!quoted) {
            console.log("❌ Aucun message cité");
            return null;
        }
        ...
        const buffer = await downloadMediaMessage(
            ...
            "buffer",
            {
                // logger obligatoire si tu ne veux pas de logs
                logger: { 
                    info() {}, 
                    error() {}, 
                    warn() {} 
        ...
        console.error("❌ ERREUR downloadQuotedMedia :", error);
        return null;
    }
}
